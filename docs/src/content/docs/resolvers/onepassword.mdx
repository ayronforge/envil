...
<TypeTable rows={[
  ...
  { name: "client", type: "{ secrets: { resolveAll } }", description: "Custom 1Password client. If omitted, the SDK is lazily imported." },
...
<Callout type="note">
  Either `client` or `serviceAccountToken` (or the `OP_SERVICE_ACCOUNT_TOKEN` environment variable) must be available. If none are provided, the resolver fails with a `ResolverError`.
</Callout>
...
The resolver authenticates using a 1Password service account token. You can provide it in three ways:
...
3. **Custom client** (for testing or advanced usage)

## Batch resolution

The resolver uses `resolveAll()` to fetch all secrets in a single batch call, which is more efficient than resolving them one by one.
  ...
  client: {
    secrets: {
      resolveAll: async (refs) => {
        // Return values in the same order as refs
        return refs.map(() => "mock-value")
      },
    },
  },
})
