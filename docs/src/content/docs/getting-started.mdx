...
<Callout type="tip" title="Framework presets">
  Using Next.js, Vite, or Expo? Check out [Framework Presets](/docs/framework-presets) for pre-configured prefix rules so you don't have to configure `NEXT_PUBLIC_` or `VITE_` prefixes manually.
</Callout>

## Next steps

- Learn about [Core Concepts](/docs/core-concepts) like client/server separation and the proxy model
- Explore the [Built-in Schemas](/docs/schemas) for URLs, numbers, booleans, and more
- Set up [Cloud Secret Resolution](/docs/resolvers) for production
