...
envil ships with a set of ready-to-use schemas for common environment variable patterns. All schemas are exported from the main package.

```ts
import { requiredString, port, boolean, url } from "@ayronforge/envil"
```
...
```ts
import { requiredString, optional } from "@ayronforge/envil"
...
<TypeTable rows={[
  { name: "number", type: "number", description: "Any number parsed from string via NumberFromString." },
...
```ts
import { number, port, positiveNumber, integer } from "@ayronforge/envil"
...
createEnv({
  server: {
    ...
    THRESHOLD: number,             // any number (e.g. "3.14" → 3.14)
  },
})
```
...
```ts
import { boolean } from "@ayronforge/envil"
...
```ts
import { url, postgresUrl, redisUrl } from "@ayronforge/envil"
...
```ts
import { commaSeparated, commaSeparatedNumbers } from "@ayronforge/envil"
...
```ts
import { stringEnum } from "@ayronforge/envil"
...
<Callout type="tip">
  If you need a schema that doesn't exist as a built-in, just use Effect Schema directly. envil accepts any `Schema.Schema<any, any, never>`.
</Callout>
