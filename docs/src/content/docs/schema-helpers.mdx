...
better-env provides four helper functions for common patterns when defining environment variable schemas.

```ts
import { withDefault, optional, redacted, json } from "@ayronforge/better-env"
```
...
```ts
import { optional } from "@ayronforge/better-env"
