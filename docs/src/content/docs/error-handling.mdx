...
description: Understand the error classes thrown by envil and how to handle them.
order: 8
---
...
Thrown when a [resolver](/envil/docs/resolvers) fails to initialize or fetch secrets. This is an Effect tagged error, used in the Effect error channel.
...
If you prefer handling errors without `try`/`catch` or Effect error channels, use [`safeCreateEnv`](/envil/docs/safe-parsing). It captures errors in a discriminated result object instead of raising exceptions:
...
<Callout type="tip">
  With resolvers, `safeCreateEnv` returns an Effect that **never fails** — both `ResolverError` and `EnvValidationError` are captured in the result object. See [Safe Parsing](/envil/docs/safe-parsing) for details.
</Callout>
